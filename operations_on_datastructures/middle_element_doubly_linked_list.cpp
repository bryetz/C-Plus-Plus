...
void LinkedList::addNode(const int value) {
  ...
  if (head == nullptr) { // first node (head) is null. Linked list is empty.
    ...
    newNode->prev = tail; // tail is the previous node to newNode, which will be the new tail
...
LinkedList::Node* LinkedList::findMiddle() { 
  // Prints value of middle node in linked list using two pointer approach (slow and fast)
  Node* slow = head;
  Node* fast = head;
  if (head != nullptr) {
    while (fast != nullptr && fast->next != nullptr) { 
      // fast and its next pointer are not null
      fast = fast->next->next; // fast pointer advances two nodes per iteration
      slow = slow->next; // slow pointer advances one node per iteration
      }

      // the slow pointer will be at the middle of the linked list once the fast pointer reaches the end.

      std::cout << "Middle element of linked list: " << slow->value << std::endl;
      return slow;
  } else {
...
static void test() {
  ...
  for(int i = 1; i < numElements; i++) { // insert nodes to populate linked list
    linked->addNode(i);
  }
